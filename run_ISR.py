...
path_data = r'...\Data'
os.chdir(path_data)
Data = pd.read_csv('data_ISR.csv', index_col = 0)
...
axs[0].plot(cH_gbr_ma.T, c='g')
axs[0].plot(true_hosp_ma, c='r')
axs[0].set_title('ISR')
axs[0].set_ylim(0,2*np.max(true_hosp_ma))
axs[1].plot(dD_mtx_gbr_ma.T, c='g')
axs[1].plot(true_deaths_ma, c='r')
axs[1].set_ylim(0,2*np.max(true_deaths_ma))
plt.show()

np.save('dH_mtx_isr.npy', dH_mtx)
np.save('dE_mtx_isr.npy', dE_mtx)
np.save('dR_mtx_isr.npy', dR_mtx)
np.save('dD_mtx_isr.npy', dD_mtx)
np.save('dAsy_mtx_isr.npy', dAsy_mtx)
np.save('dSy_mtx_isr.npy', dSy_mtx)
np.save('cH_isr.npy', cH)
np.save('dPos_isr.npy', dPos)
np.save('cumulativeDet_list_isr.npy', cumulativeDet_list)
